<main id="main" class="main">
    <div class="pagetitle">
        <h1>Medication Status Report</h1>
        <nav>
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="<?= base_url() ?>dashboard">Home</a></li>
                <li class="breadcrumb-item"><a href="<?= base_url() ?>report">Report</a></li>
                <li class="breadcrumb-item active">Medication Status</li>
            </ol>
        </nav>
    </div><!-- End Page Title -->
                ...
                <div class="col-lg-12">
                    <div class="card shadow-sm">
                        <div class="card-header d-flex justify-content-between align-items-center">
                            <h4 class="card-title mb-0">List of Medication Status</h4>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const rowData = <?= json_encode($completedIntakes); ?>;
    ...
    if (!Array.isArray(rowData) || rowData.length === 0) {
        ...
        let gridData = rowData.map(status => [
            `${status.Firstname} ${status.Lastname}`,  // Full Patient Name
            status.MedicationName,                     // Medication Name
            status.Remark,                             // Medication Status or Remark
            status.Date,                               // Status Date
            status.Time,                               // Time
            formatDate(status.created_at),             // Created At (formatted)
        ]);
    ...
    function initializeGrid(data, limit) {
        return new gridjs.Grid({
            columns: ['Patient Name', 'Medication Name', 'Remark/Status', 'Status Date', 'Time','Date Added'], // Columns for Medication Status
            data: data,
            sort: true,
            search: true,
    ...
    const filteredData = rowData.filter(status => {
        ...
        `${status.Firstname} ${status.Lastname}`,  // Patient Full Name (Firstname + Lastname)
        status.MedicationName,                     // Medication Name
        status.Remark,                             // Remark/Status
        status.Date,                               // Status Date
        status.Time,                               // Time (Separate Time Column)
        formatDate(status.created_at),             // Created At (Formatted Date)
    ]);
    ...
    function printTable() {
        ...
        printContent += '<th>Patient Name</th><th>Medication Name</th><th>Remark/Status</th><th>Status Date</th><th>Time</th><th>Date Added</th><th></tr></thead><tbody>';
